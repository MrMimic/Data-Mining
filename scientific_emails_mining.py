...
class Requests:
			...
			for a in range(0, len(plosAuthorsList)-1):
				if(re.search('email', str(plosAuthorsList[a]))):
					...
					if(plosAuthorEmail is not None):
						...
						authorName = re.findall(regexPlosName, re.sub('\n', '', re.sub(' <span class="contribute"> </span>', '', str(plosAuthorsList[a]))))[0]
						''' Get clean email '''
	...
	def getSpringerEmails(self, searchedTerm):
		...
		for pageCount in range(1,int((total/pageLength)+2)):
			springerSearch = 'http://api.springer.com/metadata/json?api_key=<YOUR_API_KEY>=50&s=' + str(start) + '&q=' + searchedTerm
